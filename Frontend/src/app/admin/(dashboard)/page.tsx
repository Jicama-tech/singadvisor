...
export default async function AdminDashboard() {
  ...
  const events = allEvents.filter((e) => e.published).length;
  const now = Date.now();
  const nextEvent =
    allEvents
      .filter((e) => e.published && new Date(e.startDate).getTime() >= now)
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-sm text-[var(--text-secondary)]">
                {formatDateTime(nextEvent.startDate)} · {nextEvent.venue}
              </p>
            </div>
            <Link
              href={`/admin/events/${nextEvent._id}`}
