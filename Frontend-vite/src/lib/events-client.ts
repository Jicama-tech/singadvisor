...
// ---------------------------------------------------------------------------
// Public reads — the SPA calls eventsh's public routes directly from the
// browser (they are unauthenticated by design; the organizer id in the URL
// is public config, see vite.config.ts). Anything needing the API key goes
// through the Backend's /eventsh/* proxy (events-admin-client.ts).
//
...
// not globally). See docs/API_CLIENT_INTEGRATION.md (eventsh repo) for the
// full rationale.
//
// Never throws — returns [] / null on any failure so pages degrade instead
// of crashing. Mirrors `fetchLandingSections` in `landing-client.ts`.
// ---------------------------------------------------------------------------
