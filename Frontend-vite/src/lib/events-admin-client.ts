...
// ---------------------------------------------------------------------------
// Events — moved to eventsh (Phase 4 API-client integration). The Next app
// called eventsh directly from its server with the organizer API key held in
// server-only env vars; a browser SPA cannot hold that secret, so every
// admin operation now goes through the Backend's transparent /eventsh/*
// proxy (Backend/src/modules/eventsh-proxy/), which attaches the key
// server-side. The SPA authenticates against the proxy with the session
// Bearer token exactly like every other admin call. Upstream status codes
// and bodies pass through verbatim, so all the error handling below is
// unchanged from the Next version.
// ---------------------------------------------------------------------------
...
async function eventshFetch(path: string, init: RequestInit = {}): Promise<unknown> {
  ...
  try {
    response = await fetch(`${__API_URL__}/eventsh${path}`, {
      ...init,
      headers: {
...
export async function fetchEventsAdmin(): Promise<EventRow[]> {
  const result = (await eventshFetch(
    `/events/organizer/${__EVENTSH_ORGANIZER_ID__}?publicOnly=false`,
 ...
 * JSON Content-Type that helper always sets. Goes through the Backend
 * proxy's dedicated multipart route (which re-wraps the file server-side
 * with the organizer key attached). */
export async function uploadEventImage(file: File): Promise<{ url: string }> {
  ...
  let response: Response;
  try {
    response = await fetch(`${__API_URL__}/eventsh/uploads/events`, {
      method: "POST",
...
export async function fetchTicketsAdmin(): Promise<TicketRow[]> {
  const organizerId = __EVENTSH_ORGANIZER_ID__;
